...
print_r(getenv('REQUEST_URI'));
if (isset ($_SESSION['admin_logged']) &&$_SESSION['admin_logged'] == true &&
isset ($_POST['product_id']))
{
if (isset ($this->_mDepartmentId) && isset ($this->_mCategoryId)){
//echo htmlspecialchars_decode(Link::ToProductAdmin($this->_mDepartmentId,$this->_mCategoryId,(int)$_POST['product_id']));exit;
...
header('Location: ' .htmlspecialchars_decode(Link::ToProductAdmin($department_id,
...
$this->mSearchDescription ='<p class="description">Products containing <font class="words">'. ($this->mAllWords == 'on' ? 'all' : 'any') . '</font>'. ' of these words: <font class="words">'. implode(', ', $search_results['accepted_words']) .
'</font></p>';
}
...
/*print_r($this->mPage);
echo'<br/>';
print_r($this->mrTotalPages);exit;*/
