...
//print_r($link);print_r(htmlspecialchars($link, ENT_QUOTES));exit;
// Escape html
...
//Array ( [0] => DepartmentId=2 [1] => CategoryId=4 ) converts to => Array ( [DepartmentId] => 2 [CategoryId] => 4 ) 
return $result;
}
...
isset ($_GET['AjaxRequest'])||/*isset ($_GET['AddProduct'])*/
isset ($_POST['Login']) ||
isset ($_GET['Logout']) ||
isset ($_GET['RegisterCustomer']) ||
isset ($_GET['RenewPassword']) ||
isset ($_GET['AddressDetails']) ||
isset ($_GET['CustomerShipping']) ||
isset ($_GET['CreditCardDetails']) ||
isset ($_GET['AccountDetails']) ||
isset ($_GET['Checkout']) ||
isset ($_GET['PaypalConfirmation']) ||
...
// Creates a link to the register customer page
public static function ToRenewPassword()
{
return self::Build('renew-password/', 'https');
}
...
// Creates a link to the update customer shipping page
public static function ToCustomerShipping()
{
return self::Build('customer-shipping/', 'https');
}
// Creates a link to the paypal confirmation page
public static function ToPaypalConfirmation()
{
return self::Build('paypal-confirmation/', 'https');
}
...
public static function ToOrderDone($orderId)
{
return self::Build('order-done/?'.$orderId);
}
